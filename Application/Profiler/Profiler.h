...
protected:
	...
	void CalculateResultAll( void);	///< 결과 생성

public:
	void TotalBegin();
	void TotalEnd();

	void BeginUnit( unsigned int uikey, const char *lpszName);
	...
	void GetResultText( unsigned int uiKey, char *szTextResult );
	float GetResultProcessMSTime( unsigned int uiKey)
	{
		CProfileUnit &unit = m_arrProfileUnits.binary_search_bykey_returnvalue( uiKey );
		return unit.ResultProfileMSTime();
	}
};
